...
export default function LogForm({ selectedLog, onClear, detailLog, onDetailClear, handleToast }: LogFormProps) {
  ...
  function handleTagKeyDown(
    ...
    if (!val) return handleToast("Tag cannot be empty", "error", "Invalid tag");
    if (current.includes(val)) return handleToast("Tag already added", "error", "Invalid tag");
    if (current.length >= TAG_MAX_COUNT) return handleToast(`Maximum ${TAG_MAX_COUNT} tags allowed`, "error", "Invalid tag");
    if (val.length > TAG_MAX_LENGTH) return handleToast(`Tag must be under ${TAG_MAX_LENGTH} characters`, "error", "Invalid tag");
    if (!/^[a-z0-9-]+$/.test(val)) return handleToast("Tag can only contain letters, numbers, and hyphens", "error", "Invalid tag");
      ...
      <div className="w-full lg:w-2/3 lg:self-start flex flex-col gap-4">
        <div className="form-entry-panel">
          ...
          <div className="form-entry-body">
            ...
            <button type="submit" id="addLogBtn" disabled={isCreating}>
              {isCreating ? "..." : "Add to log"}
            </button>
          </div>
        </div>
